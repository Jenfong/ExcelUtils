...
import jenfong.hnacode.deal.Excelsplit;
import jenfong.hnacode.deal.FreezeUtil;
...
public class FreezeUtilTest {
	private static Log log = LogFactory.getLog(FreezeUtilTest.class);

	public static void main(String[] args) {
		if (args.length == 0) throw new RuntimeException("参数输入错误！！！文件完整路径、冻结的行、冻结的列，参数以空格分开。如：\"E:/海航酒店集团酒店运营情况综合日报.xls\" 2 3");
		String filePath = args[0];
		// String filePath = "E:/海航酒店集团酒店运营情况综合日报.xls";
		File oldfile = new File(filePath);
		if(!oldfile.exists()) throw new RuntimeException("文件路径错误或文件不存在，请检查!!!");
